    ...
    async fn stream_execute(&self, mut inputs: Vec<mpsc::Receiver<StreamChunk>>, params: &HashMap<String, String>) -> mpsc::Receiver<StreamChunk> {
        ...
        let level = match params.get("level") {
            Some(v) => match v.parse::<i32>() {
                Ok(l) => l,
                Err(e) => return error_stream(format!("StreamingZstdCompress: invalid param 'level': {e}")),
            },
            None => 3,
        };
        if !(1..=22).contains(&level) {
            return error_stream(format!("StreamingZstdCompress: invalid param 'level': {} out of range 1..=22", level));
        }
    ...
    async fn stream_execute(&self, mut inputs: Vec<mpsc::Receiver<StreamChunk>>, params: &HashMap<String, String>) -> mpsc::Receiver<StreamChunk> {
        ...
        let quality = match params.get("quality") {
            Some(v) => match v.parse::<u32>() {
                Ok(q) => q,
                Err(e) => return error_stream(format!("StreamingBrotliCompress: invalid param 'quality': {e}")),
            },
            None => 4,
        };
        if quality > 11 {
            return error_stream(format!("StreamingBrotliCompress: invalid param 'quality': {} out of range 0..=11", quality));
        }
        spawn_map(rx, CAP, move |chunk| {
            let mut out = Vec::new();
            {
...
impl StreamingComputeFunction for StreamingPad {
    ...
    async fn stream_execute(&self, mut inputs: Vec<mpsc::Receiver<StreamChunk>>, params: &HashMap<String, String>) -> mpsc::Receiver<StreamChunk> {
        ...
        if block_size == 0 {
            return error_stream("StreamingPad: block_size must be > 0".into());
        }
